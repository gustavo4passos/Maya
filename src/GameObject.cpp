...
void GameObject::Draw(Renderer* renderer, float deltaTime) {
	...
	Rect dst = Rect(refacPostion, _spriteW,  _spriteH);
	Rect src = Rect(_currentFrame*_width, _currentRow*, _spriteW, _spriteH);

	if(_facingright){
		renderer->Draw(ResourceManager::GetTexture(_textureName), &src, &dest);
	}
	else{
		renderer->Draw(ResourceManager::GetTexture(_textureName), &src, &dest, true);
	}	
